...
There are two types of agents in CyxCode; primary agents and subagents.
...
CyxCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
CyxCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

CyxCode comes with two built-in primary agents and two built-in subagents.
...
If no temperature is specified, CyxCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global CyxCode config and the project specific config.
...
The model ID in your CyxCode config uses the format `provider/model-id`. For example, if you're using [CyxCode Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `cyxcode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/code3hr/cyxcode).
:::

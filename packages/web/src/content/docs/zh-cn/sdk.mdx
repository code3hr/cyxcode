---
title: SDK
description: Type-safe JS client for CyxCode server.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The CyxCode JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control CyxCode programmatically.
...
Create an instance of CyxCode:

```javascript
import { createCyxCode } from "@cyxcode/sdk"

const { client } = await createCyxCode()
```
...
const CyxCode = await createCyxCode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${CyxCode.server.url}`)

CyxCode.server.close()
```

## Client only

If you already have a running instance of CyxCode, you can create a client instance to connect to it:

```javascript
import { createCyxCodeClient } from "@cyxcode/sdk"

const client = createCyxCodeClient({
...
```typescript
import type { Session, Message, Part } from "@cyxcode/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*CyxCode" },
})

---
title: Providers
description: Using any LLM provider in CyxCode.
---

import config from "../../../../config.mjs"
export const console = config.console

CyxCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your CyxCode config.
...
You can customize the providers through the `provider` section in your CyxCode
config.
...
CyxCode Zen is a list of models provided by the CyxCode team that have been
tested and verified to work well with CyxCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with CyxCode Zen.
:::

1. Run the `/connect` command in the TUI, select `CyxCode Zen`, and head to [cyxcode.ai/auth](https://cyxcode.ai/zen).
...
It works like any other provider in CyxCode and is completely optional to use.

---

## CyxCode Go

CyxCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the CyxCode team that have been
tested and verified to work well with CyxCode.

1. Run the `/connect` command in the TUI, select `CyxCode Go`, and head to [cyxcode.ai/auth](https://cyxcode.ai/zen).
...
To use Amazon Bedrock with CyxCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running CyxCode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY CyxCode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile CyxCode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX CyxCode
   ```
...
CyxCode. Anthropic explicitly prohibits this.

Previous versions of CyxCode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in CyxCode with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for CyxCode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX CyxCode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX CyxCode
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your CyxCode config.
...
GitLab Duo support in CyxCode is experimental. Features, configuration, and
behavior may change in future releases.
:::

CyxCode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in CyxCode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
CyxCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock CyxCode
...
More documentation on [CyxCode-gitlab-auth](https://www.npmjs.com/package/CyxCode-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, CyxCode will:
...
4. Route tool execution requests through CyxCode's permission-gated tool system
...
To use your GitHub Copilot subscription with CyxCode:
...
To use Google Vertex AI with CyxCode:
...
2. Set the required environment variables:
   ...
   Set them while running CyxCode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id CyxCode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through CyxCode, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each CyxCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each CyxCode conversation listed as a separate session.
...
You can configure CyxCode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure CyxCode to use local models through LM Studio.
...
You can configure CyxCode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for CyxCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/CyxCode) for details.
:::
...
To use Ollama Cloud with CyxCode:
...
3. Copy the API key for use in CyxCode.
...
6. **Important**: Before using cloud models in CyxCode, you must pull the model information locally:
...
CyxCode Zen is a list of tested and verified models provided by the CyxCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>CyxCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **CyxCode Zen**.

   ```txt
   /connect
   ```

3. Enter your CyxCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your CyxCode config.
...
5. You can also customize them through your CyxCode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' CyxCode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group CyxCode
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with CyxCode:
...
You can also customize models through your CyxCode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with CyxCode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
The `limit` fields allow CyxCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `cyxcode auth list` to see if the credentials
...
2. For custom providers, check the CyxCode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your CyxCode config.

---
title: الويب
description: استخدام CyxCode في متصفحك.
---

يمكن تشغيل CyxCode كتطبيق ويب داخل متصفحك، ليمنحك تجربة البرمجة القوية بالذكاء الاصطناعي نفسها دون الحاجة إلى terminal.

![CyxCode Web - جلسة جديدة](../../../assets/web/web-homepage-new-session.png)
...
يؤدي ذلك إلى تشغيل خادم محلي على `127.0.0.1` بمنفذ عشوائي متاح، ويفتح CyxCode تلقائيا في المتصفح الافتراضي لديك.
...
:::tip[مستخدمو Windows]
لأفضل تجربة، شغّل `cyxcode web` من [WSL](/docs/windows-wsl) بدلا من PowerShell. يضمن ذلك وصولا صحيحا إلى نظام الملفات وتكاملا أفضل مع terminal.
:::
...
افتراضيا، يختار CyxCode منفذا متاحا. يمكنك تحديد منفذ:
...
افتراضيا، يرتبط الخادم بـ `127.0.0.1` (للاستخدام المحلي فقط). لجعل CyxCode متاحا على شبكتك:

```bash
cyxcode web --hostname 0.0.0.0
```

عند استخدام `0.0.0.0` سيعرض CyxCode كلا من عناوين الوصول المحلي وعناوين الشبكة:
...
بعد التشغيل، تتيح لك واجهة الويب الوصول إلى جلسات CyxCode الخاصة بك.

### الجلسات

اعرض جلساتك وأدرها من الصفحة الرئيسية. يمكنك رؤية الجلسات النشطة وبدء جلسات جديدة.

![CyxCode Web - جلسة نشطة](../../../assets/web/web-homepage-active-session.png)

### حالة الخادم

انقر على "See Servers" لعرض الخوادم المتصلة وحالتها.

![CyxCode Web - See Servers (عرض الخوادم)](../../../assets/web/web-homepage-see-servers.png)

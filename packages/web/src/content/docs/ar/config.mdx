---
title: الإعدادات
description: استخدام ملف إعدادات CyxCode بصيغة JSON.
---

يمكنك ضبط CyxCode باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم CyxCode تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
ضع إعدادات CyxCode العامة في `~/.config/opencode/opencode.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
عند تشغيل CyxCode، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`cyxcode.ai/config.json`**](https://cyxcode.ai/config.json).
...
يمكنك ضبط إعدادات الخادم لأوامر `cyxcode serve` و`cyxcode web` عبر الخيار `server`.
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم CyxCode.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات CyxCode عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول CyxCode استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات CyxCode عبر الخيار `theme`.
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود CyxCode إلى `"build"` مع تحذير.

ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`cyxcode run`) وتطبيق سطح المكتب وGitHub Action.
...
سيقوم CyxCode بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
[الإضافات](/docs/plugins) توسّع CyxCode بأدوات وخُطافات وتكاملات مخصصة.
...
هذا مفيد عندما تريد تقييد CyxCode لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.

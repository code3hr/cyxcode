---
title: IDE
description: امتداد CyxCode لـ VS Code و Cursor وغيرها من بيئات التطوير
---

يتكامل CyxCode مع VS Code و Cursor أو أي IDE يدعم terminal. ما عليك سوى تشغيل `opencode` في terminal للبدء.

---

## الاستخدام

- **تشغيل سريع**: استخدم `Cmd+Esc` (Mac) أو `Ctrl+Esc` (Windows/Linux) لفتح CyxCode في عرض terminal مقسّم، أو للتركيز على جلسة terminal موجودة إذا كانت قيد التشغيل بالفعل.
- **جلسة جديدة**: استخدم `Cmd+Shift+Esc` (Mac) أو `Ctrl+Shift+Esc` (Windows/Linux) لبدء جلسة terminal جديدة لـ CyxCode حتى لو كانت هناك جلسة مفتوحة. يمكنك أيضا النقر على زر CyxCode في واجهة المستخدم.
- **وعي بالسياق**: شارك تلقائيا تحديدك الحالي أو تبويبك مع CyxCode.
...
لتثبيت CyxCode على VS Code والتفرعات الشائعة مثل Cursor و Windsurf و VSCodium:
...
ابحث عن **CyxCode** في Extension Marketplace وانقر **Install**.

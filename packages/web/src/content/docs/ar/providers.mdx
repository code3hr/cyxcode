---
title: المزوّدون
description: استخدام أي موفّر LLM في CyxCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم CyxCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات CyxCode.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات CyxCode.
...
CyxCode Zen هي قائمة نماذج يوفّرها فريق CyxCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع CyxCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع CyxCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opencode، ثم انتقل إلى [cyxcode.ai/auth](https://cyxcode.ai/auth).
...
يعمل مثل أي مزوّد آخر في CyxCode واستخدامه اختياري بالكامل.

---

## CyxCode Go

CyxCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق CyxCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع CyxCode.

1. شغّل الأمر `/connect` في TUI، واختر `CyxCode Go`، ثم انتقل إلى [cyxcode.ai/auth](https://cyxcode.ai/zen).
...
لاستخدام Amazon Bedrock مع CyxCode:
...
:::info
استخدام اشتراك Claude Pro/Max داخل CyxCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم CyxCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر CyxCode
...
لاستخدام Google Vertex AI مع CyxCode:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة CyxCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة CyxCode مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع CyxCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع CyxCode:
...
3. انسخ مفتاح API لاستخدامه في CyxCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في CyxCode، يجب سحب معلومات النموذج محليا:
...
CyxCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق CyxCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>CyxCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **CyxCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ CyxCode.
...
تسمح حقول `limit` لـ CyxCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `cyxcode auth list` لمعرفة ما إذا كانت بيانات الاعتماد

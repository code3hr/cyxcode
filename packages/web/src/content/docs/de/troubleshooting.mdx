...
Wenn CyxCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `cyxcode --log-level DEBUG`.

---

## Speicher

cyxcode speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
CyxCode Desktop startet im Hintergrund einen lokalen CyxCode-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `CyxCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
CyxCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit CyxCode ihn neu aufbauen kann.

1. Beende CyxCode Desktop komplett.
...
3. Starte CyxCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

CyxCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Unter Windows benoetigt CyxCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit CyxCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

CyxCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer CyxCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende CyxCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von CyxCode Desktop):
...
Wenn du Probleme mit CyxCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `cyxcode models` aus.
...
cyxcode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).

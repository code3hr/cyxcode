---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um CyxCode zu erweitern.
---

Mit Plugins können Sie CyxCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von CyxCode zu ändern.
...
CyxCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein CyxCode-SDK-Client für die Interaktion mit AI.
...
import type { Plugin } from "@cyxcode/plugin"
...
Hier sind einige Beispiele für Plugins, mit denen Sie CyxCode erweitern können.
...
:::note
Wenn Sie die CyxCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass CyxCode `.env`-Dateien liest:
...
Plugins können CyxCode auch benutzerdefinierte Tools hinzufügen:

```ts title=".opencode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@cyxcode/plugin"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das CyxCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für CyxCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://cyxcode.ai/docs/sdk).

---
title: Web
description: Verwenden Sie CyxCode in Ihrem Browser.
---

CyxCode kann als Webanwendung in Ihrem Browser ausgeführt werden und bietet das gleiche leistungsstarke KI-Codierungserlebnis, ohne dass ein Terminal erforderlich ist.

![CyxCode Web – Neue Sitzung](../../../assets/web/web-homepage-new-session.png)
...
Dadurch wird ein lokaler Server auf `127.0.0.1` mit einem zufällig verfügbaren Port gestartet und CyxCode automatisch in Ihrem Standardbrowser geöffnet.
...
:::tip[Windows-Nutzer]
Um die beste Erfahrung zu erzielen, führen Sie `cyxcode web` über [WSL](/docs/windows-wsl) und nicht über PowerShell aus. Dies gewährleistet den ordnungsgemäßen Zugriff auf das Dateisystem und die Terminalintegration.
:::
...
Standardmäßig wählt CyxCode einen verfügbaren Port aus. Sie können einen Port angeben:
...
Standardmäßig bindet der Server an `127.0.0.1` (nur localhost). So machen Sie CyxCode in Ihrem Netzwerk zugänglich:

```bash
cyxcode web --hostname 0.0.0.0
```

Bei Verwendung von `0.0.0.0` zeigt CyxCode sowohl lokale als auch Netzwerkadressen an:
...
```bash
cyxcode web --mdns --mdns-domain myproject.local
```
...
```bash
cyxcode web --cors https://example.com
```
...
```bash
OPENCODE_SERVER_PASSWORD=secret cyxcode web
```
...
Nach dem Start bietet die Weboberfläche Zugriff auf Ihre CyxCode-Sitzungen.

### Sitzungen

Zeigen Sie Ihre Sitzungen auf der Startseite an und verwalten Sie sie. Sie können aktive Sitzungen sehen und neue starten.

![CyxCode Web – Aktive Sitzung](../../../assets/web/web-homepage-active-session.png)

### Serverstatus

Klicken Sie auf „Server anzeigen“, um verbundene Server und deren Status anzuzeigen.

![CyxCode Web – Siehe Server](../../../assets/web/web-homepage-see-servers.png)

...
:::caution
Modi werden jetzt über die Option `agent` in der CyxCode-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in CyxCode können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die CyxCode-Konfiguration.
...
CyxCode verfügt über zwei integrierte Modi.
...
Wenn keine Temperatur angegeben ist, verwendet CyxCode modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale CyxCode-Konfiguration als auch die projektspezifische Konfiguration.

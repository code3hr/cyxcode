...
Sie können externe Tools zu CyxCode hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. CyxCode unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [CyxCode Config](https://cyxcode.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
CyxCode übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird CyxCode Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von CyxCode beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `cyxcode mcp auth <server-name>` verwenden.
...
```bash
cyxcode mcp auth my-oauth-server
```
...
```bash
cyxcode mcp logout my-oauth-server
```

Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert CyxCode die Token sicher in `~/.local/share/opencode/mcp-auth.json`.
...
cyxcode mcp debug my-oauth-server
```
...
Ihre MCPs sind neben integrierten Tools auch als Tools in CyxCode verfügbar. Sie können sie also wie jedes andere Tool über die CyxCode-Konfiguration verwalten.
...
```bash
cyxcode mcp auth sentry
```

Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und CyxCode mit Ihrem Sentry-Konto zu verbinden.

...
Es gibt zwei Arten von Agenten in CyxCode; Primäragenten und Subagenten.
...
CyxCode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Wenn keine Temperatur angegeben ist, verwendet CyxCode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale CyxCode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer CyxCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [CyxCode Zen](/docs/zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `cyxcode models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/code3hr/cyxcode).
:::

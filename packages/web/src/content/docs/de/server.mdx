...
Der Befehl `cyxcode serve` startet einen headless HTTP-Server.
...
Der Benutzername ist standardmaessig `opencode`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `cyxcode serve` und `cyxcode web`.
...
Mit `cyxcode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `cyxcode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die CyxCode-[IDE](/docs/ide)-Plugins.

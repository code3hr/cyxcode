---
title: CLI
description: Opciones y comandos de la CLI de CyxCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de CyxCode de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
cyxcode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con CyxCode mediante programación.
...
Inicie la interfaz de usuario del terminal CyxCode.
...
La CLI de CyxCode también tiene los siguientes comandos.

---

### agent

Administrar agentes para CyxCode.
...
Conecte una terminal a un servidor backend CyxCode que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
cyxcode attach [url]
```

Esto permite usar TUI con un backend remoto CyxCode. Por ejemplo:
...
CyxCode funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `cyxcode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/opencode/auth.json`.

```bash
cyxcode auth login
```

Cuando se inicia CyxCode, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en CyxCode.
...
También puede conectarse a una instancia `cyxcode serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
Inicie un servidor CyxCode sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.
...
Enumere todas las sesiones CyxCode.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones CyxCode.
...
Importe datos de sesión desde un archivo JSON o una URL compartida CyxCode.

```bash
cyxcode import <file>
```

Puede importar desde un archivo local o una URL compartida CyxCode.
...
Inicie un servidor CyxCode sin cabeza con una interfaz web.

```bash
cyxcode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a CyxCode a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Desinstale CyxCode y elimine todos los archivos relacionados.
...
La CLI de CyxCode toma las siguientes banderas globales.
...
CyxCode se puede configurar mediante variables de entorno.

---
title: Proveedores
description: Usando cualquier proveedor LLM en CyxCode.
---

import config from "../../../../config.mjs"
export const console = config.console

CyxCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración CyxCode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu CyxCode
configuración.
...
CyxCode Zen es una lista de modelos proporcionados por el equipo CyxCode que han sido
probado y verificado para funcionar bien con CyxCode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con CyxCode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione opencode y diríjase a [cyxcode.ai/auth](https://cyxcode.ai/auth).
...
Funciona como cualquier otro proveedor en CyxCode y su uso es completamente opcional.

---

## CyxCode Go

CyxCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de CyxCode que han sido
probado y verificado para funcionar bien con CyxCode.

1. Ejecute el comando `/connect` en TUI, seleccione `CyxCode Go` y diríjase a [cyxcode.ai/auth](https://cyxcode.ai/zen).
...
Para usar Amazon Bedrock con CyxCode:
...
:::info
El uso de su suscripción Claude Pro/Max en CyxCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
CyxCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear CyxCode
...
Para utilizar Google Vertex AI con CyxCode:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación CyxCode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación CyxCode enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para CyxCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con CyxCode:
...
3. Copie la clave API para usarla en CyxCode.
...
6. **Importante**: Antes de usar modelos en la nube en CyxCode, debe obtener la información del modelo localmente:
...
CyxCode Zen es una lista de modelos probados y verificados proporcionada por el equipo CyxCode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>CyxCode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **CyxCode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave CyxCode API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con CyxCode:
...
Los campos `limit` le permiten a CyxCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `cyxcode auth list` para ver si las credenciales

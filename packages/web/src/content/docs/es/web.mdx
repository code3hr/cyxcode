---
title: Web
description: Usando CyxCode en su navegador.
---

CyxCode puede ejecutarse como una aplicación web en su navegador, brindando la misma poderosa experiencia de codificación de IA sin necesidad de una terminal.

![CyxCode Web - Nueva sesión](../../../assets/web/web-homepage-new-session.png)
...
Esto inicia un servidor local en `127.0.0.1` con un puerto disponible aleatorio y abre automáticamente CyxCode en su navegador predeterminado.
...
Para obtener la mejor experiencia, ejecute `cyxcode web` desde [WSL](/docs/windows-wsl) en lugar de PowerShell. Esto garantiza el acceso adecuado al sistema de archivos y la integración del terminal.
:::
...
De forma predeterminada, CyxCode elige un puerto disponible. Puede especificar un puerto:
...
De forma predeterminada, el servidor se vincula a `127.0.0.1` (solo localhost). Para hacer que CyxCode sea accesible en su red:

```bash
cyxcode web --hostname 0.0.0.0
```

Al usar `0.0.0.0`, CyxCode mostrará direcciones locales y de red:
...
Una vez iniciada, la interfaz web proporciona acceso a sus sesiones CyxCode.

### Sesiones

Vea y administre sus sesiones desde la página de inicio. Puedes ver sesiones activas e iniciar otras nuevas.

![CyxCode Web - Sesión activa](../../../assets/web/web-homepage-active-session.png)

### Estado del servidor

Haga clic en "Ver servidores" para ver los servidores conectados y su estado.

![CyxCode Web - Ver Servidores](../../../assets/web/web-homepage-see-servers.png)

---
title: Server
description: Interact with cyxcode server over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `cyxcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an CyxCode client can use.
...
```bash
cyxcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
cyxcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opencode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `cyxcode serve` and `cyxcode web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password cyxcode serve
```
...
When you run `cyxcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the cyxcode server to interact with CyxCode programmatically.
:::

This architecture lets CyxCode support multiple clients and allows you to interact with CyxCode programmatically.

You can run `cyxcode serve` to start a standalone server. If you have the
cyxcode TUI running, `cyxcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CyxCode [IDE](/docs/ide) plugins.
...
The cyxcode server exposes the following APIs.

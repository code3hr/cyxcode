---
title: CLI
description: CyxCode CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

cyxcode CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.
...
cyxcode terminal kullanıcı arayüzünü başlatır.
...
cyxcode CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

cyxcode için agent'ları yönetin.
...
cyxcode, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `cyxcode auth login` kullanabilirsiniz. Bu `~/.local/share/opencode/auth.json`'de saklanıyor.

```bash
cyxcode auth login
```

cyxcode başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `cyxcode serve` örneğine de ekleyebilirsiniz:
...
cyxcode oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
cyxcode'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
cyxcode'u en son sürüme veya belirli bir sürüme günceller.
...
cyxcode CLI aşağıdaki global bayrakları destekler.
...
cyxcode ortam değişkenleri kullanılarak yapılandırılabilir.

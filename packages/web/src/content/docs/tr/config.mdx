---
title: Yapılandırma
description: CyxCode JSON yapılandırması kullanır.
---

cyxcode'u JSON yapılandırma dosyası kullanarak yapılandırabilirsiniz.

---

## Format

cyxcode hem **JSON** hem de **JSONC** (JSON Yorumlarla birlikte) formatlarını destekler.
...
cyxcode başlatıldığında geçerli dizinde bir yapılandırma dosyası arar veya en yakın Git dizinine gider.
...
Yapılandırma dosyası [**`cyxcode.ai/config.json`**](https://cyxcode.ai/config.json)'da tanımlanan bir şemaya sahiptir.
...
`cyxcode serve` ve `cyxcode web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
cyxcode yapılandırmanızda kullanmak istediğiniz sağlayıcıları ve modelleri `provider`, `model` ve `small_model` seçenekleri aracılığıyla yapılandırabilirsiniz.
...
cyxcode yapılandırmanızda kullanmak istediğiniz temayı `theme` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`cyxcode run`), masaüstü uygulaması ve GitHub Action.
...
cyxcode başlatıldığında yeni güncellemeleri otomatik olarak indirecektir. Bunu `autoupdate` seçeneğiyle devre dışı bırakabilirsiniz.

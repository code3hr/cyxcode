...
CyxCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
CyxCode Go şu anda beta olarak mevcut.
:::

Go, CyxCode'daki diğer sağlayıcılar gibi çalışır. CyxCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve CyxCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
CyxCode ile iyi çalışan seçili bir model ve sağlayıcı grubunu test ettik.
:::
...
CyxCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

CyxCode Go, CyxCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>CyxCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `CyxCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye CyxCode Go'ya abone olabilir.
:::
...
CyxCode Go aşağıdaki limitleri içerir:
...
CyxCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.5 için yapılandırmanızda `opencode-go/kimi-k2.5` kullanmalısınız.
...
CyxCode Go'yu şu amaçlarla oluşturduk:
...
4. CyxCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.

...
cyxcode, GitHub iş akışınızla bütünleşir. Yorumunuzda `/opencode` veya `/oc`'den bahsedin; opencode, GitHub Actions çalıştırıcınızdaki görevleri yürütecektir.
...
cyxcode aşağıdaki GitHub olayları tarafından tetiklenebilir:
...
cyxcode'un davranışını iş akışınız için özelleştirmek için varsayılan istemi geçersiz kılın.

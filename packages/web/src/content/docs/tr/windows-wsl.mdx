...
cyxcode Windows'ta dogrudan calisabilir, ancak en iyi deneyim icin [Windows Subsystem for Linux (WSL)](https://learn.microsoft.com/en-us/windows/wsl/install) kullanmanizi oneriyoruz. WSL, opencode ozellikleriyle sorunsuz calisan bir Linux ortami saglar.
...
cyxcode Desktop kullanmak isteyip sunucuyu WSL'de calistirmak istiyorsaniz:
...
1. **`cyxcode web` komutunu PowerShell yerine WSL terminalinde calistirin**:
...
`cyxcode web` komutunu WSL'den calistirmak, dosya sistemi erisimi ve terminal entegrasyonunu daha tutarli hale getirirken tarayicidan erisimi korur.

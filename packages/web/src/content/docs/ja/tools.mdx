...
Tools allow the LLM to perform actions in your codebase. CyxCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in CyxCode.
...
Writes durable project knowledge into the local wiki. Notes can include `[[wikilinks]]` so later sessions can reuse discovered architecture, decisions, and code relationships.
...
Searches the project wiki and reads matching note summaries, links, and backlinks. Pass a page id to read note content.
...
:::note
This tool is only available when using the CyxCode provider or when the `OPENCODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching CyxCode:

```bash
OPENCODE_ENABLE_EXA=1 cyxcode
```

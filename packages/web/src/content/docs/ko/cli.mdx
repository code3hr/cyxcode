---
title: CLI
description: CyxCode CLI 옵션과 명령어.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

cyxcode CLI는 인수 없이 실행하면 기본적으로 [TUI](/docs/tui)를 시작합니다.
...
CyxCode는 [Models.dev](https://models.dev)의 provider 목록을 기반으로 동작하므로, `cyxcode auth login`으로 원하는 provider의 API 키를 설정할 수 있습니다. 인증 정보는 `~/.local/share/opencode/auth.json`에 저장됩니다.

```bash
cyxcode auth login
```

CyxCode 시작 시 인증 파일에서 provider 정보를 불러오며, 시스템 환경 변수나 프로젝트의 `.env`에 정의된 키도 함께 로드합니다.
...
`--refresh` 플래그를 사용하면 캐시된 모델 목록을 갱신할 수 있습니다. provider에 새 모델이 추가된 뒤 CyxCode에서 바로 확인하고 싶을 때 유용합니다.
...
매번 MCP 서버 콜드 부트가 발생하지 않도록, 실행 중인 `cyxcode serve` 인스턴스에 붙어서 실행할 수도 있습니다.
...
API 접근용 headless CyxCode 서버를 시작합니다. 전체 HTTP 인터페이스는 [server docs](/docs/server)를 참고하세요.
...
CyxCode 세션의 토큰 사용량과 비용 통계를 표시합니다.
...
JSON 파일 또는 CyxCode 공유 URL에서 세션 데이터를 가져옵니다.

```bash
cyxcode import <file>
```

로컬 파일이나 CyxCode 공유 URL에서 가져올 수 있습니다.
...
웹 인터페이스를 포함한 headless CyxCode 서버를 시작합니다.

```bash
cyxcode web
```

이 명령은 HTTP 서버를 시작하고 웹 브라우저를 열어 웹 인터페이스로 CyxCode에 접속합니다. `OPENCODE_SERVER_PASSWORD`를 설정하면 HTTP basic auth가 활성화됩니다(기본 사용자명: `opencode`).

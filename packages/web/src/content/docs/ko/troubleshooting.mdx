...
자세한 디버그 정보를 얻기 위해 `--log-level` 명령줄 옵션을 사용하여 로그 레벨을 설정할 수 있습니다. 예를 들면, `cyxcode --log-level DEBUG`.

---

## 저장소

cyxcode 저장 세션 데이터 및 디스크에 다른 응용 데이터:
...
cyxcode 데스크톱은 배경에서 로컬 opencode 서버 (`opencode-cli` sidecar)를 실행합니다. 대부분의 문제는 misbehaving 플러그인, 손상된 캐시, 또는 나쁜 서버 설정에 의해 발생합니다.

## 빠른 확인

- 완전히 종료하고 앱을 다시 시작.
- 앱이 오류 화면을 보여 주면 ** Restart**를 클릭하고 오류 세부 정보를 복사합니다.
- macOS만: `CyxCode` 메뉴 -> **웹뷰 **(UI가 공백/frozen인 경우).
...
cyxcode 데스크톱은 자체 로컬 서버(기본)를 시작하거나 구성된 서버 URL에 연결할 수 있습니다.
...
3. 당신은 `cyxcode upgrade`를 가진 최신 버전이 있는 것을 지킵니다
...
당신이 액세스 할 수있는 모델을 파악하려면, `cyxcode models`를 실행

...
cyxcode 데스크톱 앱을 쓰면서 서버는 WSL에서 실행하고 싶다면 다음처럼 진행하세요.
...
1. **PowerShell 대신 WSL terminal에서 `cyxcode web` 실행하기**
...
WSL에서 `cyxcode web`를 실행하면 올바른 파일 시스템 접근과 terminal 통합을 유지하면서도 Windows 브라우저에서 바로 사용할 수 있습니다.

...
You can add external tools to CyxCode using the _Model Context Protocol_, or MCP. CyxCode supports both local and remote servers.
...
You can define MCP servers in your [CyxCode Config](https://cyxcode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/CyxCode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
CyxCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, CyxCode will:
...
If the server requires authentication, CyxCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `cyxcode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, CyxCode will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in CyxCode, alongside built-in tools. So you can manage them through the CyxCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect CyxCode to your Sentry account.

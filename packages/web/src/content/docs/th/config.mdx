---
title: Config
description: Using the CyxCode JSON config.
---

You can configure CyxCode using a JSON config file.

---

## Format

CyxCode supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
1. **Remote config** (from `.well-known/CyxCode`) - organizational defaults
...
Organizations can provide default configuration via the `.well-known/CyxCode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/CyxCode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global CyxCode config in `~/.config/opencode/opencode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
When CyxCode starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
The server/runtime config schema is defined in [**`cyxcode.ai/config.json`**](https://cyxcode.ai/config.json).

TUI config uses [**`cyxcode.ai/tui.json`**](https://cyxcode.ai/tui.json).
...
```json title="tui.json"
{
  "$schema": "https://cyxcode.ai/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
...
You can configure server settings for the `cyxcode serve` and `cyxcode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your CyxCode server.
...
You can configure the providers and models you want to use in your CyxCode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, CyxCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, CyxCode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`cyxcode run`), desktop app, and GitHub Action.
...
CyxCode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
CyxCode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, CyxCode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend CyxCode with custom tools, hooks, and integrations.
...
This is useful when you want to restrict CyxCode to only use specific providers rather than disabling them one by one.

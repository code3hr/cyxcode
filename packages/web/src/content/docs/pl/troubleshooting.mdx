...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `cyxcode --log-level DEBUG`.

---

## Przechowywanie danych

cyxcode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
cyxcode Desktop uruchamia w tle lokalny serwer opencode (sidecar `opencode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `CyxCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
cyxcode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
cyxcode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
cyxcode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:
...
3. Upewnij się, że masz najnowszą wersję z `cyxcode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `cyxcode models`
...
cyxcode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.

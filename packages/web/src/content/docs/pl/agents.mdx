...
W CyxCode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
CyxCode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
CyxCode ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

CyxCode ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Jeśli nie zostanie skonfigurowana, CyxCode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji CyxCode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji CyxCode w `provider/model-id`. Na przykład, [CyxCode Zen](/docs/zen), użyj `opencode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `cyxcode models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/code3hr/cyxcode).
:::

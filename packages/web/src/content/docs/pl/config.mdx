---
title: Konfiguracja
description: Ustawienia CyxCode w pliku JSON.
---

Możesz dostosować CyxCode za pomocą pliku konfiguracyjnego JSON.

---

## Format

CyxCode obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Umieść swoją globalną konfigurację CyxCode w `~/.config/opencode/opencode.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.
...
Kiedy CyxCode się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`cyxcode.ai/config.json`**](https://cyxcode.ai/config.json).
...
Możesz skonfigurować serwer dla `cyxcode serve` i `cyxcode web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer CyxCode.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie CyxCode wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, CyxCode powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`cyxcode run`), aplikacji desktopowej i GitHub Actions.
...
CyxCode może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy CyxCode nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie CyxCode **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają CyxCode o niestandardowe narzędzia, hooki i integracje.
...
Jest to przydatne, gdy chcesz ograniczyć użycie CyxCode tylko do określonych dostawców.

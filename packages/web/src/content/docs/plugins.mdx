---
title: Plugins
description: Write your own plugins to extend CyxCode.
---

Plugins allow you to extend CyxCode by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify CyxCode's default behavior.
...
```json title="opencode.json"
{
  ...
  "plugin": ["opencode-helicone-session", "CyxCode-wakatime", "@my-org/custom-plugin"]
}
```
...
CyxCode runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
- `client`: An CyxCode SDK client for interacting with the AI.
...
Here are some examples of plugins you can use to extend CyxCode.
  ...
  return {
    event: async ({ event }) => {
      ...
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "CyxCode"'`
      }
    },
  }
}
...
:::note
If you’re using the CyxCode desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Prevent CyxCode from reading `.env` files:
...
Plugins can also add custom tools to CyxCode:
...
The `tool` helper creates a custom tool that CyxCode can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to CyxCode alongside built-in tools.
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://cyxcode.ai/docs/sdk) for details.

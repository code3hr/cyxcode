...
Per diagnosticare problemi con CyxCode, inizia controllando i log e i dati locali che salva su disco.
...
Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `cyxcode --log-level DEBUG`.

---

## Archiviazione

cyxcode salva i dati delle sessioni e altri dati dell'applicazione su disco in:
...
CyxCode Desktop esegue in background un server locale di CyxCode (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `CyxCode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
CyxCode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che CyxCode possa ricostruirla.

1. Chiudi completamente CyxCode Desktop.
...
3. Riavvia CyxCode Desktop.

---

### Risolvi problemi di connessione al server

CyxCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, CyxCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di CyxCode.

---

### Notifiche non visualizzate

CyxCode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per CyxCode nelle impostazioni del sistema operativo, e
...
1. Chiudi CyxCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app CyxCode Desktop):
...
Se riscontri problemi con CyxCode:
...
Per capire a quali modelli hai accesso, esegui `cyxcode models`
...
cyxcode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.

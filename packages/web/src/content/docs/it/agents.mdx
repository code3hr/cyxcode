...
In CyxCode esistono due tipi di agenti: agenti primari e subagenti.
...
CyxCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
CyxCode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

CyxCode include due agenti primari integrati e due subagenti integrati.
...
Se non specifichi una temperature, CyxCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di CyxCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione CyxCode usa il formato `provider/model-id`. Per esempio, se stai usando [CyxCode Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `cyxcode models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/code3hr/cyxcode).
:::

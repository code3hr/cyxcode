---
title: Supporto ACP
description: Usa CyxCode in qualunque editor compatibile con ACP.
---

CyxCode supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare CyxCode via ACP, configura l'editor in modo che esegua il comando `cyxcode acp`.

Il comando avvia CyxCode come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'CyxCode' nel selettore agenti di AI Chat.
...
Per usare CyxCode come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare CyxCode come agente ACP per la chat.
...
CyxCode funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione CyxCode

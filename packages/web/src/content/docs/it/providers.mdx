---
title: Provider
description: Usa qualsiasi provider LLM in CyxCode.
---

import config from "../../../../config.mjs"
export const console = config.console

CyxCode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di CyxCode.
...
CyxCode Zen e una lista di modelli forniti dal team CyxCode, testati e
verificati per funzionare bene con CyxCode. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da CyxCode Zen.
:::

1. Esegui `/connect` nella TUI, seleziona opencode e vai su [cyxcode.ai/auth](https://cyxcode.ai/auth).
...
Funziona come qualsiasi altro provider in CyxCode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con CyxCode:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in CyxCode non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
CyxCode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare CyxCode
...
Per usare Google Vertex AI con CyxCode:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di CyxCode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di CyxCode elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per CyxCode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con CyxCode:
...
3. Copia la chiave API da usare in CyxCode.
...
6. **Importante**: Prima di usare modelli cloud in CyxCode, devi scaricare le informazioni del modello localmente:
...
CyxCode Zen è una lista di modelli testati e verificati forniti dal team CyxCode. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>CyxCode Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **CyxCode Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di CyxCode.
...
I campi `limit` permettono a CyxCode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `cyxcode auth list` per vedere se le credenziali

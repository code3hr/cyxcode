...
Puoi aggiungere strumenti esterni a CyxCode usando il _Model Context Protocol_, o MCP. CyxCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [CyxCode Config](https://cyxcode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
CyxCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, CyxCode:
...
Se il server richiede autenticazione, CyxCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `cyxcode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, CyxCode memorizzera' i token in modo sicuro in `~/.local/share/opencode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in CyxCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione CyxCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare CyxCode al tuo account Sentry.

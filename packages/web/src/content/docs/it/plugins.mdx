---
title: Plugin
description: Scrivi plugin per estendere CyxCode.
---

I plugin ti permettono di estendere CyxCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di CyxCode.
...
CyxCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
:::note
Se usi l'app desktop di CyxCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://cyxcode.ai/docs/sdk) per i dettagli.

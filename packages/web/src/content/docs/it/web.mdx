---
title: Web
description: Usa CyxCode nel browser.
---

CyxCode puo funzionare come applicazione web nel browser, offrendo la stessa potente esperienza di coding AI senza bisogno di un terminale.

![CyxCode Web - Nuova sessione](../../../assets/web/web-homepage-new-session.png)
...
Questo avvia un server locale su `127.0.0.1` con una porta disponibile scelta a caso e apre automaticamente CyxCode nel browser predefinito.
...
:::tip[Utenti Windows]
Per la migliore esperienza, esegui `cyxcode web` da [WSL](/docs/windows-wsl) invece che da PowerShell. Questo garantisce accesso corretto al file system e integrazione col terminale.
:::
...
Per impostazione predefinita, CyxCode sceglie una porta disponibile. Puoi specificare una porta:
...
Per impostazione predefinita, il server si lega a `127.0.0.1` (solo localhost). Per rendere CyxCode accessibile sulla rete:

```bash
cyxcode web --hostname 0.0.0.0
```

Usando `0.0.0.0`, CyxCode mostrera sia l'indirizzo locale sia quello di rete:
...
Una volta avviata, l'interfaccia web ti da accesso alle tue sessioni CyxCode.

### Sessioni

Visualizza e gestisci le sessioni dalla homepage. Puoi vedere le sessioni attive e avviarne di nuove.

![CyxCode Web - Sessione attiva](../../../assets/web/web-homepage-active-session.png)

### Stato del server

Fai clic su "See Servers" per vedere i server connessi e il loro stato.

![CyxCode Web - See Servers](../../../assets/web/web-homepage-see-servers.png)

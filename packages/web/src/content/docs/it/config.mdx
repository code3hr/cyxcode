...
description: Usa la configurazione JSON di CyxCode.
---

Puoi configurare CyxCode usando un file di configurazione JSON.

---

## Formato

CyxCode supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
Metti la configurazione globale di CyxCode in `~/.config/opencode/opencode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Quando CyxCode si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Il file di configurazione ha uno schema definito in [**`cyxcode.ai/config.json`**](https://cyxcode.ai/config.json).

La configurazione TUI usa [**`cyxcode.ai/tui.json`**](https://cyxcode.ai/tui.json).
...
Puoi configurare le impostazioni del server per i comandi `cyxcode serve` e `cyxcode web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server CyxCode.
...
Puoi configurare provider e modelli da usare in CyxCode tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, CyxCode prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, CyxCode fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`cyxcode run`), app desktop e GitHub Action.
...
CyxCode scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono CyxCode con strumenti personalizzati, hook e integrazioni.
...
E utile quando vuoi limitare CyxCode a usare solo alcuni provider invece di disabilitarli uno per uno.

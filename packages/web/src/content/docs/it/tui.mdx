---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di CyxCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CyxCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire CyxCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di CyxCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a CyxCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da CyxCode. _Alias_: `/quit`, `/q`

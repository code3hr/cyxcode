---
title: Plugins
description: Écrivez vos propres plugins pour étendre CyxCode.
---

Les plugins vous permettent d'étendre CyxCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de CyxCode.
...
CyxCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
:::note
Si vous utilisez l'application de bureau CyxCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://cyxcode.ai/docs/sdk) pour plus de détails.

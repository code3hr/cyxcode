---
title: Configuration
description: Utilisation de la configuration JSON CyxCode.
---

Vous pouvez configurer CyxCode à l'aide d'un fichier de configuration JSON.

---

## Format

CyxCode prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
Placez votre configuration globale CyxCode dans `~/.config/opencode/opencode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.
...
Lorsque CyxCode démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Le fichier de configuration a un schéma défini dans [**`cyxcode.ai/config.json`**](https://cyxcode.ai/config.json).

La configuration TUI utilise [**`cyxcode.ai/tui.json`**](https://cyxcode.ai/tui.json).
...
Vous pouvez configurer les paramètres du serveur pour les commandes `cyxcode serve` et `cyxcode web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur CyxCode.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration CyxCode via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, CyxCode essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, CyxCode reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`cyxcode run`), application de bureau et GitHub Action.
...
CyxCode téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
[Plugins](/docs/plugins) étendent CyxCode avec des outils, des hooks et des intégrations personnalisés.
...
Ceci est utile lorsque vous souhaitez restreindre CyxCode à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.

...
Il existe deux types d'agents dans CyxCode ; agents primaires et sous-agents.
...
CyxCode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
CyxCode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

CyxCode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Si aucune température n'est spécifiée, CyxCode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale CyxCode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration CyxCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [CyxCode Zen](/docs/zen), vous utiliserez `opencode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `cyxcode models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/code3hr/cyxcode).
:::

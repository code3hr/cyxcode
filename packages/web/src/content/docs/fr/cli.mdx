---
title: CLI
description: Options et commandes de la CLI CyxCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI CyxCode démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
cyxcode
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec CyxCode par programme.
...
Démarrez l’interface utilisateur du terminal CyxCode.
...
La CLI CyxCode dispose également des commandes suivantes.

---

### agent

Gérer les agents pour CyxCode.
...
Attachez un terminal à un serveur backend CyxCode déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
cyxcode attach [url]
```

Cela permet d'utiliser le TUI avec un backend CyxCode distant. Par exemple :
...
CyxCode est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `cyxcode auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/opencode/auth.json`.

```bash
cyxcode auth login
```

Lorsque CyxCode démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans CyxCode.
...
Vous pouvez également vous connecter à une instance `cyxcode serve` en cours d'exécution pour éviter les temps de démarrage à froid du serveur MCP à chaque exécution :
...
Démarrez un serveur CyxCode sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
cyxcode serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'CyxCode sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Liste toutes les sessions CyxCode.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions CyxCode.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage CyxCode.

```bash
cyxcode import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage CyxCode.
...
Démarrez un serveur CyxCode sans interface graphique avec une interface Web.

```bash
cyxcode web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à CyxCode via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Désinstallez CyxCode et supprimez tous les fichiers associés.
...
CyxCode peut être configuré à l'aide de variables d'environnement.

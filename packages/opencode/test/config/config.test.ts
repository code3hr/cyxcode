...
const managedConfigDir = process.env.CYXCODE_TEST_MANAGED_CONFIG_DIR!
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["CYXCODE_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      opencode: {
        options: {
          apiKey: "{env:CYXCODE_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["CYXCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["CYXCODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("does not try to install dependencies in read-only CYXCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  process.env.CYXCODE_CONFIG_DIR = tmp.extra
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "opencode.json"),
        JSON.stringify({ $schema: "https://cyxcode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
// Note: preload.ts sets CYXCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("CYXCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["CYXCODE_DISABLE_PROJECT_CONFIG"]
    process.env["CYXCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["CYXCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["CYXCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["CYXCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CYXCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["CYXCODE_CONFIG_DIR"]
      process.env["CYXCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["CYXCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["CYXCODE_CONFIG_DIR"]
      } else {
        process.env["CYXCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("CYXCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["CYXCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CYXCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["CYXCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["CYXCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from CYXCODE_CONFIG_DIR, not project
...
describe("CYXCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in CYXCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CYXCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["CYXCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://cyxcode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["CYXCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["CYXCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in CYXCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CYXCODE_CONFIG_CONTENT"]

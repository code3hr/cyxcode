...
afterEach(async () => {
  delete process.env.CYXCODE_CONFIG
  delete process.env.CYXCODE_TUI_CONFIG
...
test("project config takes precedence over CYXCODE_TUI_CONFIG (matches CYXCODE_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.CYXCODE_TUI_CONFIG = custom
    },
  })
...
test("CYXCODE_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from CYXCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.CYXCODE_CONFIG = path.join(customDir, "opencode.json")
    },
  })

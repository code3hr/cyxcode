...
describe("ProviderTransform.providerOptions", () => {
  ...
  test("uses xai sdk key for xAI provider options", () => {
    const model = createModel({
      providerID: "custom-xai",
      api: {
        id: "grok-4",
    ...
    expect(ProviderTransform.providerOptions(model, { promptCacheKey: "session-123" })).toEqual({
      xai: { promptCacheKey: "session-123" },
    })
  })
...
describe("ProviderTransform.schema - gemini type arrays", () => {
  ...
  test("splits a multi-type array into anyOf", () => {
    const schema = {
      type: "object",
      properties: {
  ...
  test("lifts null into nullable for nullable type arrays", () => {
    const schema = {
      type: "object",
      properties: {
  ...
  test("collapses all-null type arrays to null", () => {
    const schema = {
      type: "object",
      properties: {
        nothing: { type: ["null"] },
...
describe("ProviderTransform.schema - openai supported schema subset", () => {
  ...
  test("removes unsupported JSON Schema keywords recursively", () => {
    const result = ProviderTransform.schema(openai, {
  ...
  test("keeps references and sanitizes definitions", () => {
    const result = ProviderTransform.schema(openai, {
      type: "object",
      properties: {
        value: {
  ...
  test.each([
    ...
    ["azure", "@ai-sdk/azure"],
  ])("sanitizes %s models using %s", (provider, npm) => {
    const result = ProviderTransform.schema(
      {
        providerID: provider,
        api: {

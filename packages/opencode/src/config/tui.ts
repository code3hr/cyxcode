...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.CYXCODE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.CYXCODE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.CYXCODE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".opencode") && dir !== Flag.CYXCODE_CONFIG_DIR) continue

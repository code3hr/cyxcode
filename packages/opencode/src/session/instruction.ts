...
function globalFiles() {
  ...
  if (Flag.CYXCODE_CONFIG_DIR) {
    files.push(path.join(Flag.CYXCODE_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.CYXCODE_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.CYXCODE_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no CYXCODE_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.CYXCODE_CONFIG_DIR, Flag.CYXCODE_CONFIG_DIR).catch(() => [])
}
...
export namespace InstructionPrompt {
  ...
  export async function system() {
    ...
    const fetches = urls.map((url) =>
      Http.fetch(url, { signal: AbortSignal.timeout(5000) })

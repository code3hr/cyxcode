...
declare const CYXCODE_LIBC: string | undefined
  ...
  const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
    try {
      const binding = require(
        `@parcel/watcher-${process.platform}-${process.arch}${process.platform === "linux" ? `-${CYXCODE_LIBC || "glibc"}` : ""}`,
      )
    ...
    Effect.gen(function* () {
      const state = yield* InstanceState.make(
        Effect.fn("FileWatcher.state")(
          function* () {
            if (yield* Flag.CYXCODE_EXPERIMENTAL_DISABLE_FILEWATCHER) return
            ...
            if (yield* Flag.CYXCODE_EXPERIMENTAL_FILEWATCHER) {
